...
- Keyboard control supports arrows or `h` `j` `k` `l` for movement, `Cmd+C` / `Ctrl+C` to copy the selected word while the board is active, `d` for the study-info details view, `t` for the `Dictionary API` tab, `y` for `M-W Dictionary`, `u` for `M-W Thesaurus`, `g` for learned, and `r` for forgotten.
- The currently selected details tab is remembered within the session, so moving to another word keeps the same reference mode active until the learner changes it.
...
- Tapping the word title inside the details panel copies that word directly.
