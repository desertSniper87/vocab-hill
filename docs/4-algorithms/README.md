...
`lib/src/pages/home_page.dart:L471-L569` — `_HomePageState._handleBoardKeyEvent` — maps arrows and `d` / `t` / `y` / `u` / `g` / `r` onto the selected cell so learners can move, open any reference tab from the keyboard, and classify words without leaving the board.
